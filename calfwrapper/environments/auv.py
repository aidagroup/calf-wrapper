"""Contaminated-Zone AUV Navigation environment."""
...
class ContaminatedZoneAUVEnv(gym.Env):
    metadata = {
    ...
    def __init__(
        ...
        # ─── ContaminatedZoneAUVEnv.__init__ ───
    ...
    def reset(
        ...
        if seed is not None:
            self.drone = _AUV(seed=seed, init_x=self.init_x, init_y=self.init_y)
        elif self.rng is not None:
            self.drone = _AUV(
                random_generator=self.rng, init_x=self.init_x, init_y=self.init_y
            )
        else:
            self.drone = _AUV(init_x=self.init_x, init_y=self.init_y)
...
class _AUVMetrics(MetricsCollector):
