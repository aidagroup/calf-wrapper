"""Treasure-Collecting Robot environment."""
...
from calfwrapper.environments.metrics import MetricsCollector
...
class _RobotEnvironment(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        *,
        ...
        if self.config.control_dt <= 0.0:
            raise ValueError("_RobotEnvironment control_dt must be positive.")
    ...
    def _draw_robot_body(self, center: tuple[int, int], scale: int) -> None:
        ...
        pygame.draw.line(
            ...
            (
                body_rect.left + int(body_w * 0.22),
                body_rect.bottom - int(body_h * 0.22),
            ),
            width=stripe_w,
        )
        pygame.draw.line(
            ...
            (
                body_rect.right - int(body_w * 0.22),
                body_rect.bottom - int(body_h * 0.22),
            ),
            width=stripe_w,
        )
        ...
        smile_rect.center = (
            visor_rect.centerx,
            visor_rect.centery + int(body_radius * 0.30),
        )
        pygame.draw.arc(
            sprite,
            glow,
...
@dataclass
class TreasureCollectingRobotConfig(_RobotConfig):
    """Configuration for the Treasure-Collecting Robot environment."""

    const_speed: float | None = None


class TreasureCollectingRobotEnv(_RobotEnvironment):
    """Treasure-Collecting Robot with a one-dimensional steering action."""

    def __init__(
        self,
        *,
        render_mode: str | None = None,
        config: TreasureCollectingRobotConfig | None = None,
        seed: int | None = None,
    ) -> None:
        config = config or TreasureCollectingRobotConfig()
        ...
        self.action_space = spaces.Box(
            low=np.array([-config.max_angular_velocity], dtype=np.float32),
            high=np.array([config.max_angular_velocity], dtype=np.float32),
            dtype=np.float32,
        )
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict]:
        angular_velocity = float(
            np.clip(
                np.asarray(action, dtype=np.float32).flatten()[0],
                -self.config.max_angular_velocity,
                self.config.max_angular_velocity,
            )
        )
...
class _RobotMetrics(MetricsCollector):
