...
class Args:
    ...
    log_every: int = 1_000
    output_dir: Path = TRAINING_OUTPUT / "td3-lagrangian-auv"
...
def cost_probability_loss(logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
...
class Actor(nn.Module):
    def __init__(self, observation_size: int, action_space: gym.spaces.Box):
        ...
        self.register_buffer(
            "action_scale",
            torch.as_tensor((action_space.high - action_space.low) / 2.0, dtype=torch.float32),
        )
        self.register_buffer(
            "action_bias",
            torch.as_tensor((action_space.high + action_space.low) / 2.0, dtype=torch.float32),
...
def run_training(args: Args) -> None:
    ...
    if (args.env_id, args.horizon) not in supported_tasks:
        raise ValueError(
            f"unsupported environment/horizon pair for TD3-Lagrangian: {args.env_id}/{args.horizon}"
        )
    ...
    metrics_path = args.output_dir / f"td3_lagrangian_{args.environment}_seed{args.seed}.jsonl"
    metrics_path.write_text("")
    checkpoint_path = args.output_dir / f"td3_lagrangian_{args.environment}_seed{args.seed}.pt"
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step >= args.learning_starts and replay_buffer.size >= args.batch_size:
            ...
            with torch.no_grad():
                ...
                next_cost_q = target_cost_q(data["next_observations"], next_action).squeeze(-1)
                cost_target = cost_bellman_target(data["costs"], data["dones"], next_cost_q)
                require_probability("cost Bellman targets", cost_target)
                require_finite("TD3 targets", reward_target, cost_target, next_action)

            reward_q1_value = reward_q1(data["observations"], data["actions"]).squeeze(-1)
            reward_q2_value = reward_q2(data["observations"], data["actions"]).squeeze(-1)
            ...
            cost_q_logits = cost_q.logits(data["observations"], data["actions"]).squeeze(-1)
            ...
            if global_step % args.policy_frequency == 0:
                ...
                actor_reward_value = reward_q1(data["observations"], actor_action).mean()
                ...
                reward_actor_gradient_norm = tensor_gradient_norm(reward_actor_gradients)
                ...
                with torch.no_grad():
                    for parameter, target_parameter in zip(
                        actor.parameters(), target_actor.parameters(), strict=True
                    ):
                        target_parameter.mul_(1.0 - args.tau).add_(parameter, alpha=args.tau)
                ...
                latest_diagnostics.update(
                    {
                        ...
                        "cost_q_out_of_range_fraction": float(
                            ((cost_q_value < 0.0) | (cost_q_value > 1.0)).float().mean().item()
                        ),
                        "cost_q_saturation_fraction": float(
                            ((cost_q_value < 0.01) | (cost_q_value > 0.99)).float().mean().item()
                        ),
                        "cost_target_out_of_range_fraction": float(
                            ((cost_target < 0.0) | (cost_target > 1.0)).float().mean().item()
                        ),
        ...
        while (
            ...
            raw_failure_estimate, failure_estimate = estimate_initial_failure_probability(
                actor, cost_q, initial_tensor
            )
    ...
    result_path = args.output_dir / f"td3_lagrangian_{args.environment}_seed{args.seed}.json"
...
def main(args: Args) -> None:
    ...
    run_name = args.mlflow_run_name or (f"td3-lagrangian__{args.environment}__seed-{args.seed}")
